...
{
    public class RavagerPack : ModItem
    {
        ...
        public override void ModifyItemLoot(ItemLoot itemLoot) {
            AddItem(itemLoot, "FleshyGeode", 100);
        }

        public override void RightClick(Player player) {
            player.GetModPlayer<Common.ModIntegration.ModIntegrationPlayer>().SetFlag("Calamity_Ravager");
        }

        public override void AddRecipes() {
            int ingr = CalamityItemHelper.GetItemType("FleshyGeode");
            if (ingr > 0) {
                CreateRecipe()
